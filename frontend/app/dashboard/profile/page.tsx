...
export default function ProfilePage() {
  ...
  if (!user) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#EEEEEE" }}>
        <div style={{ width: 32, height: 32, borderRadius: "50%", border: "2px solid #f0f0f0", borderTop: `2px solid ${BRAND_RED}`, animation: "spin 0.8s linear infinite" }} />
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#EEEEEE", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .profile-input:focus { border-color: ${BRAND_RED}; box-shadow: 0 0 0 3px rgba(218,11,0,0.1); }
        .profile-input.err { border-color: #C0392B; background: #FDF8F8; }
        .profile-input::placeholder { color: #bbb; }
        .pw-wrap { position: relative; }
        .pw-wrap .profile-input { padding-right: 46px; }
        .pw-toggle { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: #aaa; display: flex; padding: 0; transition: color 0.15s; }
        .pw-toggle:hover { color: #1C1C1C; }
        .tab-btn { background: none; border: none; border-bottom: 2px solid transparent; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 0.875rem; font-weight: 500; padding: 0.65rem 0; color: #aaa; transition: all 0.18s; display: inline-flex; align-items: center; gap: 7px; }
        .tab-btn.active { color: #1C1C1C; border-bottom-color: ${BRAND_RED}; }
        .tab-btn:hover:not(.active) { color: #555; }
        .save-btn { height: 46px; padding: 0 28px; border: none; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 0.875rem; font-weight: 700; cursor: pointer; transition: all 0.18s; display: inline-flex; align-items: center; gap: 8px; background: ${BRAND_RED}; color: #fff; letter-spacing: 0.02em; }
        ...
        .avatar-img { width: 92px; height: 92px; border-radius: 50%; object-fit: cover; border: 2.5px solid #f0f0f0; display: block; }
        .avatar-initials { width: 92px; height: 92px; border-radius: 50%; background: ${BRAND_RED}; display: flex; align-items: center; justify-content: center; font-family: 'Playfair Display', serif; font-size: 1.7rem; font-weight: 700; color: #fff; border: 2.5px solid #ffffff; }
        .avatar-edit-btn { position: absolute; bottom: 0; right: 0; width: 28px; height: 28px; border-radius: 50%; background: ${BRAND_RED}; border: 2px solid #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #fff; transition: opacity 0.15s; }
        .avatar-edit-btn:hover { opacity: 0.85; }
        .card { background: #fff; border-radius: 16px; border: 1px solid #f0f0f0; padding: 2rem; }
      ...
      <div style={{ borderBottom: "1px solid #f0f0f0", background: "#fff" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 58 }}>
          <Link href="/dashboard" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "0.8rem", color: "#888", textDecoration: "none", transition: "color 0.15s" }}
            onMouseEnter={e => (e.currentTarget.style.color = BRAND_RED)}
            onMouseLeave={e => (e.currentTarget.style.color = "#888")}
          >
            <IconArrowLeft /> Back to dashboard
          </Link>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.15rem", fontWeight: 700, color: "#1C1C1C", letterSpacing: "0.01em" }}>VillaBaas</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "2.5rem 2rem 5rem" }}>
        ...
        <div className="card" style={{ display: "flex", alignItems: "center", gap: "1.5rem", marginBottom: "1.25rem", animation: "fadeIn 0.3s ease both" }}>
            ...
            {imageFile && imagePreview
              ...
              : (
                <div className="avatar-initials" style={{ fontSize: "1.7rem" }}>
                  {initials || (
                    <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.85)" strokeWidth="1.4">
          ...
          <div>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.55rem", fontWeight: 700, color: "#1C1C1C", marginBottom: "0.15rem" }}>
              {user.firstName} {user.lastName}
            </h1>
            <p style={{ fontSize: "0.82rem", color: "#888", marginBottom: "0.55rem" }}>{user.email}</p>
            <span style={{ display: "inline-block", fontSize: "0.63rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: BRAND_RED, background: "rgba(218,11,0,0.08)", border: "1px solid rgba(218,11,0,0.22)", borderRadius: 100, padding: "3px 10px" }}>
              {user.role ?? "Member"}
            </span>
          </div>
        </div>
        ...
        {/* Tab nav */}
        <div style={{ display: "flex", gap: "2rem", borderBottom: "1px solid #f0f0f0", marginBottom: "1.5rem", paddingLeft: "0.1rem" }}>
        ...
        {activeTab === "profile" && (
          <div className="card" style={{ animation: "fadeIn 0.22s ease both" }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.25rem", fontWeight: 700, color: "#1C1C1C", marginBottom: "0.2rem" }}>Profile details</h2>
            <p style={{ fontSize: "0.8rem", color: "#888", marginBottom: "1.75rem" }}>Update your name and email address.</p>
            ...
            <form onSubmit={handleProfileSubmit} noValidate>
              ...
              <div style={{ paddingTop: "1.25rem", borderTop: "1px solid #f5f5f5", display: "flex", alignItems: "center", gap: "1rem" }}>
        ...
        {activeTab === "password" && (
          <div className="card" style={{ animation: "fadeIn 0.22s ease both" }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.25rem", fontWeight: 700, color: "#1C1C1C", marginBottom: "0.2rem" }}>Change password</h2>
            <p style={{ fontSize: "0.8rem", color: "#888", marginBottom: "1.75rem" }}>Choose a strong password with an uppercase letter and a number.</p>
              ...
              <div style={{ background: "#fafafa", border: "1px solid #f0f0f0", borderRadius: 10, padding: "14px 16px", marginBottom: "1.75rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px" }}>
                {pwRules.map(({ rule, met }) => (
                  <div key={rule} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", flexShrink: 0, background: met ? "#27ae60" : "transparent", border: met ? "none" : "1.5px solid #ddd", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", color: "#fff" }}>
                      {met && <IconCheck size={10} />}
                    </div>
                    <span style={{ fontSize: "0.75rem", color: met ? "#27ae60" : "#aaa", transition: "color 0.2s" }}>{rule}</span>
                  </div>
                ))}
              </div>

              <div style={{ paddingTop: "1.25rem", borderTop: "1px solid #f5f5f5" }}>
