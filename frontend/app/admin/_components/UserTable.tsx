    ...
    return (
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.75rem", gap: "1rem" }}>
                <div>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2.2rem", fontWeight: 600, color: "#1a1a1a", marginBottom: 4 }}>Users</h2>
                    <p style={{ fontSize: "0.8rem", color: "#aaa" }}>{total} total users</p>
                </div>
                <a href="/admin/users/create" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem", gap: "1rem", flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <select
                        ...
                        style={{ ...inp, width: 80, padding: "0 10px" }}
                    >
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
                        <thead>
                            <tr style={{ background: "#FAF7F2", borderBottom: "1px solid #ebebeb" }}>
                                {["Full Name", "Email", "Username", "Role", "Joined", "Actions"].map(h => (
                                    <th key={h} style={{
                        ...
                        <tbody>
                            {data?.length ? data.map((u, i) => (
                                ...
                                >
                                    <td style={{ padding: "14px 16px", color: "#1a1a1a", fontWeight: 500 }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                            {u.profileImage ? (
                                                <img src={`${API_URL}${u.profileImage}`} alt="" style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover" }} />
                                            ) : (
                                                <div style={{
                                                    width: 30, height: 30, borderRadius: "50%",
                                                    background: "linear-gradient(135deg,#C9A96E,#e8c97a)",
                                                    display: "flex", alignItems: "center", justifyContent: "center",
                                                    fontSize: "0.65rem", fontWeight: 700, color: "#1a1a1a", flexShrink: 0,
                                                }}>
                                    ...
                                    <td style={{ padding: "14px 16px", color: "#666" }}>{u.username || "—"}</td>
                                    <td style={{ padding: "14px 16px" }}>
                                        <span style={{
                                            ...
                                            letterSpacing: "0.08em", textTransform: "uppercase",
                                            background: u.role === "admin" ? "rgba(201,169,110,0.15)" : "#f5f5f5",
                                            color: u.role === "admin" ? "#8B6914" : "#888",
                                        }}>
                                            {u.role}
                                        </span>
                                    </td>
                                    <td style={{ padding: "14px 16px", color: "#aaa", fontSize: "0.78rem" }}>
                                        {u.createdAt ? new Date(u.createdAt).toLocaleDateString() : "—"}
                                    </td>
                                    <td style={{ padding: "14px 16px" }}>
                                        <div style={{ display: "flex", justifyContent: "flex-end", gap: 12 }}>
                                            <a href={`/admin/users/${u._id}`} style={{ fontSize: "0.75rem", fontWeight: 600, color: "#888", textDecoration: "none", letterSpacing: "0.06em", textTransform: "uppercase" }}
                                                onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = "#1a1a1a")}
                                                onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = "#888")}
                                            >View</a>
                                            <a href={`/admin/users/${u._id}/edit`} style={{ fontSize: "0.75rem", fontWeight: 600, color: "#888", textDecoration: "none", letterSpacing: "0.06em", textTransform: "uppercase" }}
                                                onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = "#1a1a1a")}
                                                onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = "#888")}
                                            >Edit</a>
                                            <button onClick={() => setTarget(u)} style={{ fontSize: "0.75rem", fontWeight: 600, color: "#888", background: "none", border: "none", cursor: "pointer", letterSpacing: "0.06em", textTransform: "uppercase", padding: 0, fontFamily: "'DM Sans', sans-serif" }}
                                ...
                                <tr>
                                    <td colSpan={6} style={{ padding: "3rem", textAlign: "center", color: "#aaa", fontSize: "0.88rem" }}>
                                        No users found
                                    </td>
                                </tr>
                            )}
            ...
            <Modal open={!!target} onClose={() => { setTarget(null); setDeleteError(""); }} title="Delete User">
                <p style={{ fontSize: "0.88rem", color: "#555", lineHeight: 1.7, marginBottom: "1.25rem" }}>
                    Are you sure you want to delete{" "}
                    <strong style={{ color: "#1a1a1a" }}>{target?.firstName} {target?.lastName}</strong>?
                    This action cannot be undone.
                </p>
                {deleteError && (
                    <p style={{ fontSize: "0.8rem", color: "#C0392B", marginBottom: "1rem" }}>{deleteError}</p>
                )}
